...
def run_timeloop(DIR, LAYER_DIR, layer = "pw"):
    ...
    if not debug_timeloop_output:
        p = subprocess.Popen(['timeloop-mapper', accelerator + '/baseline/arch.yaml', accelerator + '/baseline/components/smartbuffer_RF.yaml',
                              accelerator + '/baseline/components/smartbuffer_SRAM.yaml', mapper, LAYER_DIR+'prob/prob.yaml',
                              LAYER_DIR+'constraints/arch_constraints.yaml',\
                     LAYER_DIR+'constraints/map_constraints.yaml', '-o', LAYER_DIR+'output'], stdin=subprocess.PIPE,
        ...
        subprocess.run(['timeloop-mapper', accelerator+'/baseline/arch.yaml', accelerator+'/baseline/components/smartbuffer_RF.yaml',
                     accelerator+'/baseline/components/smartbuffer_SRAM.yaml',\
...
def fix_layer_shape_forward(BLOCK_DIR, layer, block, bottleneck=False):
    ...
    if layer == "conv" or layer == "max_pool" or layer == "dw":
