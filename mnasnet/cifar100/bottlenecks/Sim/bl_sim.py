...
def fix_layer_shape(BLOCK_DIR, layer, block):
    ...
    global bl_in_ch, bl_midd_ch, bl_out_ch, bl_pixels, bl_stride, bl_kernel_size
    ...
    if layer == 'conv':
        ...
        prob['problem']['instance']['R'] = bl_kernel_size[block]
        prob['problem']['instance']['S'] = bl_kernel_size[block]
...
def sparse_baseline_sim():
    ...
    p = subprocess.Popen(['rm', 'csvs/*', 'convert_comp.py', 'convert_decomp.py', 'temp_91.csv', 'temp.csv'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    p.wait()
...
def sim(delay, mapper, dw_mapper, blocks, in_ch, midd_ch, out_ch, pixels, stride, CSV_only, sparse_only, kernel_size):
