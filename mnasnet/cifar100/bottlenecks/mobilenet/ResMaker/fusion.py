...
def adjust_pred(file, IN_CHANNELS, MIDD_CHANNELS, qtz=4, pred1_qtz=False, pred2_qtz=False, decomp=True, block=1, q_blocks=10):
    global ADDERS
    conv = not decomp
...
def get_extra_energy(file, fused_weights, block):
    ...
    weight_DRAM =  fused_weights * DRAM_ACCESS_ENERGY * INDEX_FACTOR
    if block < 4:
        weight_DRAM *= 16
    elif block < 8:
        weight_DRAM *= 4
