...
def get_sparse_blcok_energy(DIR, block):
    ...
    for layer in ['decom', 'conv', 'comp']:
        ...
        if layer == 'decom':
            sparse_stats[0].append(e)
        elif layer == 'conv':
            sparse_stats[1].append(e)
        else:
            sparse_stats[2].append(e)
...
def get_layer_stats(file, layer, MIDD_CHANNELS):
...
def get_blcok_stats(DIR, block):

    global stats, midd_channels

    for layer in ['decom', 'conv', 'comp']:
        file = DIR + "b" +str(block+1) + "/" + layer + "/output/timeloop-mapper.stats.txt"
        e, c = get_layer_stats(file, layer, midd_channels[block])
        if layer == 'decom':
            stats[0].append(e)
            stats[1].append(c)
        elif layer == 'conv':
...
def make_csv(blocks, DIR, midd_channels_bl):

    global stats
    global midd_channels
    midd_channels = midd_channels_bl

    for block in range(blocks):
        get_blcok_stats(DIR, block)

    df = pandas.DataFrame(data={"1_PW1_e": stats[0], "2_DW_e": stats[2], "3_PW2_e": stats[4],\
                                "4_PW1_c": stats[1], "5_DW_c": stats[3], "6_PW2_c": stats[5]})
    df.to_csv("results/bl_res.csv", sep=',',index=False)


    for block in range(blocks):
        get_sparse_blcok_energy(DIR, block)
    sparse_csv()
