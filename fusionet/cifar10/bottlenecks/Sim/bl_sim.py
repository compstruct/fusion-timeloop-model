...
def fix_layer_shape(BLOCK_DIR, layer, block):
    ...
    if layer == 'conv':
        ...
        prob['problem']['instance']['C'] = int(bl_in_ch[block] / BL_GROUPS)
        if block == 0:
             prob['problem']['instance']['M'] = int((bl_midd_ch[block]-24) / BL_GROUPS)
        elif bl_in_ch[block] == bl_out_ch[block] :
            prob['problem']['instance']['M'] = int(bl_midd_ch[block] / BL_GROUPS)
        else:
            prob['problem']['instance']['M'] = int((bl_midd_ch[block]-bl_midd_ch[block-1]) / BL_GROUPS)

    else:
        prob['problem']['instance']['C'] = int(bl_midd_ch[block] / BL_GROUPS)
        prob['problem']['instance']['M'] = int(bl_out_ch[block] / BL_GROUPS)

    if layer == 'decom' or layer == 'comp':
...
def sim(CSV_only, delay, mapper, dw_mapper, blocks, groups, in_ch, midd_ch, out_ch, pixels, stride, sparse_only=False):

    global DELAY, mapper_pars, dw_mapper_pars
    global BL_BLOCKS, BL_GROUPS
    global bl_in_ch, bl_midd_ch, bl_out_ch, bl_pixels, bl_stride
    ...
    if not CSV_only:
        if not sparse_only:
            dense_baseline_sim()
        sparse_baseline_sim()
    baseline.make_csv(BL_BLOCKS, "./baseline/", BL_GROUPS, bl_midd_ch)
