...
network_head = {
  'baseline' : "head_and_tail/baseline/head/results/bl.csv",
  'fused' : "head_and_tail/fused/fus_head/results/bl.csv",
}
...
network_tail = {
  'baseline' : "head_and_tail/baseline/tail/results/bl.csv",
  'fused' : "head_and_tail/fused/fus_tail/results/bl.csv",
}

head_bl = pd.read_csv(network_head["baseline"])
head_fus = pd.read_csv(network_head["fused"])
bn = pd.read_csv(bottlenecks, error_bad_lines=False)
...
col2 = ['0']*len(head_bl["2_energy"].tolist()) +  bn["3_DW_e"].tolist()[:bottlenecks_columns] + ['0']*len(tail_bl["2_energy"].tolist())
col3 = ['0']*len(head_bl["2_energy"].tolist()) +  bn["2_PW2_e"].tolist()[:bottlenecks_columns] + ['0']*len(tail_bl["2_energy"].tolist())

col4 = head_bl["3_cycles"].tolist() + bn["4_PW1_c"].tolist()[:bottlenecks_columns] + tail_bl["3_cycles"].tolist()
col5 = ['0']*len(head_bl["3_cycles"].tolist()) +  bn["6_DW_c"].tolist()[:bottlenecks_columns] + ['0']*len(tail_bl["3_cycles"].tolist())
col6 = ['0']*len(head_bl["3_cycles"].tolist()) +  bn["5_PW2_c"].tolist()[:bottlenecks_columns] + ['0']*len(tail_bl["3_cycles"].tolist())
