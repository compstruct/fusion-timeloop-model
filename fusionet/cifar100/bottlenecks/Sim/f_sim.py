...
def fix_layer_shape(BLOCK_DIR, layer, block):
    ...
    if layer == 'decom':
        ...
        prob['problem']['instance']['Wstride'] = 1
        prob['problem']['instance']['Hstride'] = 1

    else:
        prob['problem']['instance']['C'] = 1
        prob['problem']['instance']['M'] = int(f_midd_ch[block])
...
def fusion_pred_sim(DIR):
    ...
    for block in range(F_BLOCKS):
        ...
        t1 = time.time()
        run_timeloop(PRED_DIR, PRED_BLOCK_DIR+"conv/", True)
        t2 = time.time()
