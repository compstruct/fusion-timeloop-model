...
def get_layer_stats_dw(file, layer="", block=0):
...
def make_csv(blocks, DIR, in_channels, out_channels, pixs, stride, model_desc, name):
    ...
    for block, layer in enumerate(model_description):
        ...
        if (layer == "max_pool" or layer == "ave_pool"):
            ...
            e, c = get_layer_stats_dw(DIR + names[block] +"/output/timeloop-mapper.stats.txt", block=block)
        else:
            e, c = get_layer_stats(DIR + names[block] +"/output/timeloop-mapper.stats.txt", layer)
        total_e.append(e)
        total_c.append(c)
