...
class PE:
    ...
    def process_layer(self, layer, cycles, total_PE, PE_adders):
        ...
        prediction_buffer_writes = ceil( (layer['midd_ch']+layer['in_ch']) / self.prediction_buffer.bandwidth) * ceil((layer['pixels']**2)/total_PE)
        prediction_buffer_reads =  (PE_adders)  * cycles + 1
        ...
        total_adds = ceil(PE_adders * cycles * 0.5)
        self.adders.inc_action(action_Add, total_adds)

        total_bit_masks = ceil(PE_adders * cycles * 0.5)
        self.bit_masks_and.inc_action(1, total_bit_masks)

        total_mux_ = ceil(PE_adders * cycles * 0.5)
        self.mux_.inc_action(1, total_mux_)


        total_mux = ceil(PE_adders * 2 * cycles * 0.5)
...
class Chip:
    ...
    def process(self, layer, cycles, outer_M, PE_adders):
        prediction_glb_writes = ceil( ( (layer['midd_ch'] + layer['in_ch']) * ((layer['pixels'])*(layer['pixels'])) )/self.prediction_glb.bandwidth)
        ...
        total_ands = ceil(cycles * 16 * PE_adders)
