...
def adjust_pred(file, IN_CHANNELS, groups, block, pred1_qtz, pred2_qtz, qtz, qtz_blocks, decomp=True):
    global ADDERS
    comp = not decomp
    ...
    #adjusting ternary MACs and ADDs
    total_MACs_adjusted = total_MACs/IN_CHANNELS
    total_ADDs = total_MACs

    #writing prediction masks
    pred_DRAM_energy = total_fmaps*DRAM_ACCESS_ENERGY/DRAM_WIDTH

    #Adjusting energy of prediction
    adjusted_e = raw_e * 1000000 # go to pico joules

    if comp:
        if pred2_qtz and block < qtz_blocks:
            adjusted_e -= (1.00 - PRED_FMAP_BITRATIO) * DRAM_ifmaps_e #energy for reading ifmaps from DRAM
            adjusted_e -= (1.00 - PRED_FMAP_BITRATIO) * glb_ifmap_e   #energy for reading ifmaps from GLB
            adjusted_e -= (1.00 - PRED_FMAP_BITRATIO) * ifmap_buffer_e   #energy for reading from ifmap_buffer

        adjusted_e -= DRAM_ofmaps_e    #energy for writing fmaps to DRAM
        #adjusted_e -= glb_ofmap_e  #don't write fmaps even to GLB for second prediction

    else:
        if pred2_qtz and block < qtz_blocks:
            adjusted_e -= (1.00 - PRED_FMAP_BITRATIO) * DRAM_ofmaps_e    #energy for writing ofmaps to DRAM

        if pred1_qtz and block < qtz_blocks:
            adjusted_e -= (1.00 - PRED_FMAP_BITRATIO) * DRAM_ifmaps_e #energy for reading ifmaps from DRAM
            adjusted_e -= (1.00 - PRED_FMAP_BITRATIO) * glb_ifmap_e #energy for reading ifmaps from GLB

    adjusted_e += pred_DRAM_energy  #energy for writing pred masks to DRAM
    adjusted_e -= PRED_EFF*DRAM_weights_e  #adjusted energy for fetching 2bit weights from DRAM
    adjusted_e -= PRED_EFF*spad_weights_e  #adjustedenergy for fetching 2bit weights from SPAD
    adjusted_e -= MAC_e  #energy for MACs
    adjusted_e *= 0.000001 #go back to micro joules
    adjusted_e += MAC_ENERGY*total_MACs_adjusted + ADD_ENERGY*total_ADDs #add overhead of ADD & MACs
    adjusted_e *= groups #multiply by group number for group convolution

    #cycles adjusted for number of adders per PE and groups in group convolution
    adjusted_c = int(math.floor(int(txt[txt.index('Summary Stats')+3].split()[1]) * groups / ADDERS))

    #for debug
    #if comp:
    #    print(pred_DRAM_energy, (PRED_FMAP_BITRATIO) * DRAM_ifmaps_e, DRAM_ofmaps_e, PRED_EFF * DRAM_weights_e,\
    #           PRED_EFF * spad_weights_e, MAC_e, MAC_ENERGY*total_MACs_adjusted + ADD_ENERGY*total_ADDs, adjusted_e)
    #else:
    #    print(pred_DRAM_energy, DRAM_ifmaps_e, (PRED_FMAP_BITRATIO) * DRAM_ofmaps_e, PRED_EFF * DRAM_weights_e,\
    #           PRED_EFF * spad_weights_e, MAC_e, MAC_ENERGY*total_MACs_adjusted + ADD_ENERGY*total_ADDs, adjusted_e)
...
def get_extra_energy(file, fused_weights, block):
    ...
    ind = 4
    pred_DRAM = float(txt[ind].split()[1])*8.00
...
def make_csv(BLOCKS, groups, pes, PE_adders, in_ch, midd_ch, out_ch, pixels, stride, pruning, pred1_qtz, pred2_qtz, qtz, qtz_blocks):
    ...
    for block in range(BLOCKS):
        ...
        in_ch = f_in_ch[block]
        e, c = adjust_pred(file, in_ch, groups, block, pred1_qtz, pred2_qtz, qtz, qtz_blocks)
        decom_e.append(e)
        decom_c.append(c)
    ...
    conv_c = []
    for block in range(BLOCKS):
        file = "fusion/predictions/b" + str(block+1) + "/comp/output/timeloop-mapper.stats.txt"
        in_ch = f_in_ch[block]
        e, c = adjust_pred(file, in_ch, groups, block, pred1_qtz, pred2_qtz, qtz, qtz_blocks, decomp=False)
        conv_e.append(e)
        conv_c.append(c)
    ...
    for block in range(BLOCKS):
        ...
        fused_weights = f_in_ch[block]/groups * f_midd_ch[block]/groups * 9 * f_out_ch[block] * prune_factors[block]
        e = get_extra_energy(file, fused_weights, block)
        extra_energy.append(e)
        print("fusion overhead energy for block " + str(block+1))


    df = pandas.DataFrame(data={"1_pred_decom_e": decom_e, "2_pred_comp_e": conv_e, "3_main_e": main_e, "4_overhead_e": extra_energy,\
                                "5_pred_decom_c": decom_c, "6_pred_comp_c": conv_c, "7_main_c": main_c})
