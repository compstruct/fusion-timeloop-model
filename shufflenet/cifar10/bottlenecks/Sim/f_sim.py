...
def fix_layer_shape(BLOCK_DIR, layer, block):
    ...
    if layer == 'decom':
        prob['problem']['instance']['C'] = int(f_in_ch[block] / F_GROUPS)
        prob['problem']['instance']['M'] = int(f_midd_ch[block] / F_GROUPS)
    else:
        prob['problem']['instance']['C'] = int(f_midd_ch[block] / F_GROUPS)
        prob['problem']['instance']['M'] = int(f_out_ch[block] / F_GROUPS)
...
def fix_layer_constraints(BLOCK_DIR, layer, block):
    ...
    for i in range(len(const['architecture_constraints']['targets'])):
        ...
        if current_const['target'] == 'shared_glb' and current_const['type'] == 'spatial':
            const['architecture_constraints']['targets'][i]['factors'] = "N=1 R=1 S=1 M=1"

        elif current_const['target'] == 'DummyBuffer' and current_const['type'] == 'spatial':
            const['architecture_constraints']['targets'][i]['factors'] = "N=1 R=1 S=1 M=1 C=1 Q=1 P=8"
...
def fix_block_constraints(BLOCK_DIR, block):
    ...
    fix_layer_constraints(BLOCK_DIR, 'comp', block)
...
def run_timeloop(DIR, LAYER_DIR, dw=False):
    ...
    p = subprocess.Popen(['timeloop-mapper', './../accelerator/fusion/predictions/arch.yaml', './../accelerator/fusion/predictions/components/smartbuffer_RF.yaml', './../accelerator/fusion/predictions/components/smartbuffer_SRAM.yaml',\
                     mapper, LAYER_DIR+'prob/prob.yaml', LAYER_DIR+'constraints/constraints.yaml', '-o', LAYER_DIR+'output'],\
...
def fusion_pred_sim(DIR):
    ...
    for block in range(F_BLOCKS):
        ...
        t2 = time.time()
        print("fusion predictions pw1 block ", block+1, " and took ", t2-t1)

        t1 = time.time()
        run_timeloop(PRED_DIR, PRED_BLOCK_DIR+"comp/")
        t2 = time.time()
        print("fusion predictions pw2 block ", block+1, " and took ", t2-t1)
...
def fix_block_constraints_main(BLOCK_DIR, block):
    ...
    for i in range(len(const['architecture_constraints']['targets'])):
        ...
        if current_const['target'] == 'shared_glb' and current_const['type'] == 'temporal':
            const['architecture_constraints']['targets'][i]['factors'] = 'N=1 R=1 S=1 Q=1 C=' + str(int(f_in_ch[block]/F_GROUPS)) + ' M=' + str(f_out_ch[block])

        elif current_const['target'] == 'shared_glb' and current_const['type'] == 'spatial':
            const['architecture_constraints']['targets'][i]['factors'] = 'N=1 R=1 S=1 C=1 M=1 P=' + str(8) + ' Q=1'
...
def sim(CSV_only, delay, main_mapper, pred_mapper, pred_dw_mapper, pes, xpes, spatial_x, spatial_y, blocks,\
        groups, in_ch, midd_ch, out_ch, pixels, stride, adders, prune_factors, pred1_qtz, pred2_qtz, qtz, qtz_blocks):

    global DELAY, main_mapper_pars, pred_mapper_pars, pred_dw_mapper_pars,\
           F_PEs, X_PEs, PRED_SPATIAL_GLB, PRED_SPATIAL_DUMMY, F_GROUPS,\
    ...
    if not CSV_only:
        fusion_sim()
    fusion.make_csv(F_BLOCKS, F_GROUPS, F_PEs, adders, f_in_ch, f_midd_ch, f_out_ch, f_pixels,\
                    f_stride, prune_factors, pred1_qtz, pred2_qtz, qtz, qtz_blocks)
